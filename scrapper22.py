...
def scrape_all_pages(base_url, total_pages, start_page=1, output_file='preply_tutors_lugandaAHMED.csv'):
    total_tutors = 0
...
def get_subject_slugs(session):
    """
    Dynamically fetch ALL subject slugs from language-tutors page
    """
    print("Fetching subject slugs from SEO page...")

    url = "https://preply.com/en/online/language-tutors"
    response = session.get(url, headers=HEADERS, timeout=20)

    if response.status_code != 200:
        print("Failed to fetch subjects page.")
        return []
    ...
    for a in soup.find_all("a", href=True):
        ...
        if href.startswith("/en/online/") and href.endswith("-tutors"):
            slug = href.replace("/en/online/", "").replace("-tutors", "")
            subject_slugs.add(slug)

    subject_slugs = list(subject_slugs)

    print(f"Found {len(subject_slugs)} subjects.")
...
if __name__ == "__main__":
    ...
    subject_slugs = get_subject_slugs(session)

    for slug in subject_slugs:
        base_url = f"https://preply.com/en/online/{slug}-tutors"

        print(f"\nScraping subject: {slug}")

        scrape_all_pages(
            base_url=base_url,
            total_pages=1,
            start_page=1,
            output_file="preply_all_subjectsAhmed.csv"
        )